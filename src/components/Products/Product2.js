...
function Product2(props) {    
  return (
    ...
    <section className="product" >
        ...
        <NavLink className="product__link" to={`/card/${ArrProducts[props.idProduct].id}`}>

        <div className="product__box">
             <div className="product__box-images">
                <img src={ArrProducts[props.idProduct].imgProduct} className="product__images" alt="Изображение продукта"/>
                <span className={props.hiddenProperties === 'true'? 'displayNone' : 'product__sales'}>{ArrProducts[props.idProduct].productSales}</span>
            </div>

            <div className="product__box-price">
                <div className="product__price">
                    <span className="product__price-cart">{ArrProducts[props.idProduct].productPriceCart} ₽</span>
                ...
                <div className={props.hiddenProperties === 'true'? 'displayNone' : 'product__price'}>
                    <span className="product__price-nocart">{ArrProducts[props.idProduct].productNoCart} ₽</span>
                    <span className="product__title">Обычная</span>
                </div>
            </div>

            <div className="product__name">
                <span >{ArrProducts[props.idProduct].productName}</span>
            </div>


            <div className="product__box-rating">
                {ArrProducts.slice(0, calculateAverageRating(ArrProducts[props.idProduct].id)).map((product) => ( //вывод рейтинг продукта
                    <span className="product__star product__star_on" key={product.id}></span>
                ))}

                {ArrProducts.slice(0, 5 - calculateAverageRating(ArrProducts[props.idProduct].id)).map((product) => ( //вывод затемнённых звёзд если у товара не 5 звёзд рейтинга
